...
- Edit code in this repository directory.
- Studio is runtime; `wp-content/{themes,plugins,mu-plugins}` will be symlinked to this repo.
- Updating plugins/themes locally via WP Admin will modify this repo due to symlinks.
- Commit and push `pressable-dev` to deploy to Pressable dev.
- Live is updated via Pressable Data Transfer (dev -> live).
- Content/media moves via Distributor; `uploads` is not committed or deployed.
